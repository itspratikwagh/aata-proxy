...
// ============================================================
// CWID MAGIC LINK + APEX CALLOUT SECRETS
// CWID_LINK_SECRET must match AATA_Settings__c.CWID_Link_Secret__c in SF
// (Apex signs the links, we verify). PROXY_APEX_SECRET must match
// AATA_Settings__c.Proxy_Shared_Secret__c (Apex sends it as X-AATA-Secret
// on compose-* callouts so outsiders can't burn our Claude tokens).
// ============================================================
const crypto = require("crypto");
const CWID_LINK_SECRET = process.env.CWID_LINK_SECRET || "";
const PROXY_APEX_SECRET = process.env.PROXY_APEX_SECRET || "";

function cwidSig(contactId) {
  return crypto.createHmac("sha256", CWID_LINK_SECRET).update(contactId).digest("hex");
}
function cwidSigValid(contactId, sig) {
  if (!CWID_LINK_SECRET || !contactId || !sig) return false;
  const expected = cwidSig(contactId);
  try {
    return crypto.timingSafeEqual(Buffer.from(expected, "hex"), Buffer.from(String(sig), "hex"));
  } catch (e) {
    return false; // malformed hex / wrong length
  }
}
...
- **TEXAS-track students**: their program (Texas Workforce Commission funding via HCC, coordinated by Patrick Kratochvil) has NO Foothill step, NO CWID, and NO $325 book fee — NEVER mention Foothill, CWID, or book fees to a TX student, not even to say they don't apply (they're California-only concepts a TX student has never heard of). Just say their training is fully funded through the Texas Workforce Commission. If the facts show a registered class, confirm they're enrolled in that class (name + start date). For anything specific — paperwork, funding, schedule questions — direct them to Patrick Kratochvil at (281) 676-0356 or patrickaata@gmail.com.
...
// ============================================================
// CWID MAGIC LINK — form page + submit
// Links are generated by Apex (sendEnrollmentEmails) as:
//   {PROXY_BASE}/cwid?c={contactId}&s={HMAC-SHA256(contactId, secret)}
// The HMAC means only someone holding the emailed link can reach the
// form or submit for that Contact — no enumeration by contact id.
// ============================================================
const CWID_PAGE_CSS = `
  body{font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;
    background:#f1f5f9;margin:0;display:flex;justify-content:center;align-items:flex-start;min-height:100vh}
  .card{background:#fff;max-width:430px;width:92%;margin-top:8vh;border-radius:16px;
    box-shadow:0 8px 30px rgba(15,23,42,.08);padding:28px 26px;text-align:center}
  .logo{font-size:34px;margin-bottom:6px}
  h1{font-size:19px;color:#0f172a;margin:0 0 6px}
  p{font-size:14px;color:#475569;line-height:1.55;margin:8px 0}
  input{width:100%;box-sizing:border-box;padding:13px 16px;font-size:18px;letter-spacing:2px;
    text-align:center;border:1.5px solid #cbd5e1;border-radius:10px;outline:none;margin:14px 0 4px}
  input:focus{border-color:#00AEEF;box-shadow:0 0 0 4px rgba(0,174,239,.12)}
  button{width:100%;padding:13px;font-size:15px;font-weight:600;color:#fff;border:none;
    border-radius:10px;background:linear-gradient(135deg,#00AEEF,#6366f1);cursor:pointer;margin-top:12px}
  .err{color:#b91c1c;font-size:13px;min-height:18px;margin:4px 0 0}
  .ok{font-size:42px}
  .hint{font-size:12px;color:#94a3b8}
`;

function cwidPage(inner) {
  return `<!DOCTYPE html><html><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>AATA — Foothill CWID</title><style>${CWID_PAGE_CSS}</style></head>
<body><div class="card">${inner}</div></body></html>`;
}

app.get("/cwid", async (req, res) => {
  const contactId = String(req.query.c || "");
  const sig = String(req.query.s || "");
  res.set("Content-Type", "text/html; charset=utf-8");
  res.set("X-Robots-Tag", "noindex");

  if (!cwidSigValid(contactId, sig)) {
    return res.status(403).send(cwidPage(
      `<div class="logo">✈️</div><h1>Link not valid</h1>
       <p>This link is expired or incomplete. Please use the link from your AATA email,
       or reply to that email with your 8-digit CWID instead.</p>`));
  }

  // Look up first name + whether CWID is already on file (idempotent UX)
  let firstName = "", already = "";
  try {
    const r = await sfQuery(
      `SELECT FirstName, Foothill_CWID__c FROM Contact WHERE Id = '${contactId.replace(/'/g, "")}' LIMIT 1`);
    if (r.records && r.records.length) {
      firstName = r.records[0].FirstName || "";
      already = r.records[0].Foothill_CWID__c || "";
    }
  } catch (e) {
    console.error("[cwid] lookup failed:", e.message);
  }

  if (already) {
    return res.send(cwidPage(
      `<div class="ok">✅</div><h1>Already on file${firstName ? ", " + escHtml(firstName) : ""}!</h1>
       <p>We already have your Foothill CWID (<strong>${escHtml(already)}</strong>).
       If that's wrong, reply to our email and we'll fix it.</p>`));
  }

  res.send(cwidPage(
    `<div class="logo">✈️</div>
     <h1>${firstName ? "Hi " + escHtml(firstName) + "! " : ""}Enter your Foothill CWID</h1>
     <p>Your CWID is the <strong>8-digit number</strong> in your
     "Welcome to Foothill College" email.</p>
     <p class="hint">Heads up: your CWID is all numbers. If your ID has letters in it,
     that's your CCCID — not the one we need.</p>
     <form method="POST" action="/api/submit-cwid">
       <input type="hidden" name="c" value="${escHtml(contactId)}">
       <input type="hidden" name="s" value="${escHtml(sig)}">
       <input name="cwid" inputmode="numeric" autocomplete="off" maxlength="8"
              placeholder="12345678" required>
       <button type="submit">Submit CWID</button>
     </form>`));
});

app.post("/api/submit-cwid", express.urlencoded({ extended: false }), async (req, res) => {
  if (!rateLimit(req, res, {
    endpoint: "submit-cwid", windowMs: 60 * 60 * 1000, max: 10,
    message: "Too many attempts — please try again in an hour or reply to our email instead.",
  })) return;

  const contactId = String(req.body.c || "");
  const sig = String(req.body.s || "");
  const cwidRaw = String(req.body.cwid || "").trim();
  res.set("Content-Type", "text/html; charset=utf-8");

  if (!cwidSigValid(contactId, sig)) {
    return res.status(403).send(cwidPage(
      `<h1>Link not valid</h1><p>Please use the link from your AATA email.</p>`));
  }

  // Strict: exactly 8 digits. Letters => almost certainly their CCCID.
  if (!/^\d{8}$/.test(cwidRaw)) {
    const hasLetters = /[a-zA-Z]/.test(cwidRaw);
    return res.status(400).send(cwidPage(
      `<div class="logo">🤔</div><h1>That doesn't look like a CWID</h1>
       <p>${hasLetters
          ? "That looks like your <strong>CCCID</strong> — your Foothill <strong>CWID is all numbers</strong> (8 digits). You can find it in your “Welcome to Foothill College” email."
          : "Your CWID is exactly <strong>8 digits</strong>. Please double-check and try again."}</p>
       <p><a href="/cwid?c=${encodeURIComponent(contactId)}&s=${encodeURIComponent(sig)}">← Try again</a></p>`));
  }

  try {
    const upd = await sfUpdate("Contact", contactId, { Foothill_CWID__c: cwidRaw });
    if (upd.status !== 204 && upd.status !== 200) {
      throw new Error("SF update failed: " + JSON.stringify(upd.body).slice(0, 300));
    }
    // Chatter audit note (best-effort)
    try {
      await sfCreate("FeedItem", {
        ParentId: contactId,
        Body: `Foothill CWID ${cwidRaw} submitted by student via magic link form.`,
      });
    } catch (e) { console.warn("[cwid] chatter note failed:", e.message); }

    res.send(cwidPage(
      `<div class="ok">🎉</div><h1>Got it — thank you!</h1>
       <p>Your Foothill CWID <strong>${escHtml(cwidRaw)}</strong> is saved.
       You're all set on this step.</p>`));
  } catch (err) {
    console.error("[cwid] submit error:", err.message);
    res.status(500).send(cwidPage(
      `<h1>Something went wrong</h1>
       <p>We couldn't save your CWID just now. Please reply to our email with your
       8-digit CWID and we'll record it for you.</p>`));
  }
});

// ============================================================
// CLAUDE COMPOSE ENDPOINTS — called by Salesforce Apex (not browsers).
// Auth: X-AATA-Secret header must match PROXY_APEX_SECRET. These exist
// so reminder emails are differently worded every time and replies can
// address what the student actually said.
// ============================================================
app.post("/api/compose-reminder", async (req, res) => {
  if (!apexSecretValid(req)) return res.status(401).json({ error: "unauthorized" });
  const {
    firstName, reminderNumber, classStartDate, spotsRemaining,
    cwidMissing, amountPaid, amountDue, cwidLink, lastReplyText,
  } = req.body || {};
  try {
    const outstanding = [];
    if (cwidMissing) outstanding.push("their 8-digit Foothill CWID (all numbers — if their ID has letters, that's the CCCID, not what we need)");
    if ((amountDue || 0) > 0) outstanding.push(`book fee balance of $${amountDue} (of the $325 total; $${amountPaid || 0} received so far)`);

    const sys = `You write short, warm reminder emails for AATA (American Aerospace Technical Academy) students enrolled in the free NDT training program. Output STRICT JSON: {"subject": "...", "htmlBody": "..."} — nothing else. htmlBody is simple inline-styled HTML (no external CSS), 90-160 words, signed "AATA Team". Rules:
...
- Never invent facts, dates, or amounts beyond what's given. Subject must ALWAYS be exactly "Re: Your AATA enrollment — next steps" (threading).`;
    ...
    const r = await client.messages.create({
      ...
      max_tokens: 800,
      system: sys,
      messages: [{ role: "user", content: `Compose the email for ${firstName || "the student"}.` }],
    });
    const text = (r.content && r.content[0] && r.content[0].text) || "";
    const parsed = JSON.parse(text.slice(text.indexOf("{"), text.lastIndexOf("}") + 1));
    if (!parsed.subject || !parsed.htmlBody) throw new Error("missing fields in model output");
    res.json(parsed);
  } catch (err) {
    console.error("[compose-reminder] failed:", err.message);
    // Apex falls back to its static template on any non-200
    res.status(502).json({ error: err.message });
  }
});

app.post("/api/compose-student-reply", async (req, res) => {
  if (!apexSecretValid(req)) return res.status(401).json({ error: "unauthorized" });
  const { firstName, emailBody, context } = req.body || {};
  try {
    const sys = `You write a single, helpful reply email (plain text, 60-140 words, signed "AATA Team") for an AATA NDT-program student who emailed us. Context about their enrollment: ${String(context || "none").slice(0, 500)}.
Rules: answer what they asked using ONLY the context given; if you don't know, say someone from AATA will follow up and suggest calling 323-761-9066. If their question involves their Foothill CWID, remind them it's the 8-digit all-numbers ID from the "Welcome to Foothill College" email (letters = CCCID, not it). Output the email body text only — no subject, no JSON.`;
    const r = await client.messages.create({
      model: "claude-haiku-4-5-20251001",
      max_tokens: 500,
      system: sys,
      messages: [{ role: "user", content: `Student${firstName ? " " + firstName : ""} wrote:\n\n${String(emailBody || "").slice(0, 2000)}` }],
    });
    const body = (r.content && r.content[0] && r.content[0].text) || "";
    if (!body.trim()) throw new Error("empty model output");
    res.json({ body: body.trim() });
  } catch (err) {
    console.error("[compose-student-reply] failed:", err.message);
    res.status(502).json({ error: err.message });
  }
});
